...
const AdminDashboard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Event creation/update failed"); // FIXED: Replaced invalid window() call
      console.error(err);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this event...?")) {
      try {
        // FIXED: Replaced localhost URL with BASE_URL
        ...
        // FIXED: Fixed syntax error in alert string concatenation
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div className="bg-zinc-900 p-8 rounded-[2.5rem] border border-zinc-800">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              className="w-full py-4 bg-white text-black font-black rounded-full uppercase tracking-widest hover:bg-cyan-400 transition-all mt-4"
            >
                    ...
                    <td className="p-6">
                      <div className="flex gap-3">
                        <button
                          ...
                          className="px-4 py-2 bg-cyan-500/10 text-cyan-400 rounded-lg hover:bg-cyan-500 hover:text-white transition-all text-xs font-bold"
                        >
                          EDIT
                        </button>
                        <button
                          onClick={() => handleDelete(ev._id)}
                          className="px-4 py-2 bg-red-500/10 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition-all text-xs font-bold"
                        >
                          DELETE
                        </button>
                      </div>
